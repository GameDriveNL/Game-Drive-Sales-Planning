...
export async function GET(request: NextRequest) {
  ...
  try {
    // Per-platform gate + rotation. One game per run, all games every ~7 days.
    const gate = await apifyCronGate(supabase, 'instagram')
    ...
    // Instagram sources for the rotation target only
